...
A participant asked about the name of the repo, here's how an LLM could respond:

> - "CS Unplugged" — a well-known educational approach that teaches computer science concepts stripped down to their core, without relying on computers. Similarly, this repo teaches how agents work by stripping away framework abstractions so you understand the underlying principles.
> - "The Matrix" sense — getting unplugged means seeing the raw reality behind the illusion. Building an agent from scratch lets you see what frameworks actually do under the hood, rather than treating them as black boxes.
> - "Unplugged" as in not yet connected — the agent starts as a blank slate, not yet plugged into tools, APIs, or the outside world. The course is the process of plugging it in, piece by piece.
> - Hype detox — stepping away ("unplugging") from the noise and marketing around AI agent frameworks, going back to basics to understand what actually matters.

> The MTV Unplugged reading is probably the most intuitive one, but the CS Unplugged angle fits especially well given the educational context.
